...
## 5. Créer l’admin (plan Render **Free** — pas de Shell)

### Méthode A — Depuis votre PC (recommandé)

1. Render → base **`chezwizi-db`** → copier **External Database URL**
2. Sur votre PC :
   ```bash
   cd restaurant_backend
   pip install -r requirements.txt
   ```
3. PowerShell (une seule session) :
   ```powershell
   $env:DATABASE_URL = "collez-l-external-database-url-ici"
   $env:DJANGO_DEBUG = "false"
   python manage.py createsuperuser
   ```
4. Choisir username / mot de passe → utilisables sur `/admin/` et dans l’app Flutter.

### Méthode B — Variables Render (sans Shell)

1. Web Service → **Environment** → ajouter temporairement :
   - `DJANGO_SUPERUSER_USERNAME` = `admin`
   - `DJANGO_SUPERUSER_PASSWORD` = un mot de passe fort
   - `DJANGO_SUPERUSER_EMAIL` = `admin@chezwizi.com` (optionnel)
2. **Save** → attendre le redéploiement (le `build.sh` crée l’utilisateur).
3. **Supprimer** `DJANGO_SUPERUSER_PASSWORD` (et les autres) des variables, puis **Save** encore.

> Si l’utilisateur existe déjà, le deploy ne plante pas (`|| true` dans `build.sh`).

### Plan payant (Starter+)

Shell disponible : `python manage.py createsuperuser`

---

## 6. Après le premier déploiement réussi

1. Vérifier les **Logs** : `migrate` OK, pas d’erreur `psycopg` / `DATABASE_URL`
2. Admin créé (méthode A ou B ci-dessus)
